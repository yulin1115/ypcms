...
class Oauth{
    ...
    public function qq_login($backurl){
	...
	$this->recorder->write('backurl',$backurl);
	...
	//getqqinfo为后来添加，原先api没有此函数，获取qq用户信息
	public function getqqinfo($accesstoken,$openid){

        $keysArr = array(
            "access_token" => $accesstoken,
            "oauth_consumer_key" =>"100384296",
            "openid" => $openid,
        );		
		$graph_url = $this->urlUtils->combineURL(self::GET_QQ_URL, $keysArr); 
        $qqinfo = $this->urlUtils->get_contents($graph_url);
		return $qqinfo;
	}
}
