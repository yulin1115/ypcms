...
class Page {
    ...
    protected $coolPages   ;
    // 分页显示定制
    protected $config  =    array('header'=>'条记录','prev'=>'上一页','next'=>'下一页','first'=>'第一页','last'=>'最后一页','theme'=>' %totalRow% %header% %nowPage%/%totalPage% 页 %upPage% %downPage% %first%  %prePage%  %linkPage%  %nextPage% %end% %jump%');
    // 默认分页变量名
    protected $varPage;
	//是否显示输入页数跳转
    ...
    public function show() {
		...
		if($this->isJumpPage){
			...
			$jump .='<input type=\'button\' class=\'submit1\' value=\'GO\' onclick=\'go_page('.$this->nowPage.')\'>';
			...
			if($this->AjaxType){
				...
				$jump.='var url=\''.$url.'\';';
				//$jump .='window.location.href = \''.$url.C('URL_PATHINFO_DEPR').$p.C('URL_PATHINFO_DEPR').'\'+topage+\''.C('URL_HTML_SUFFIX').'\'';
				$jump .='window.location.href =url.replace(\'__PAGE__\',topage);';
				//echo $jump;
			}
			$jump .='}}</script>';
		}		
